 ...
 */
let runtimeTable = (() => { const t = {}; for (const [id, c] of Object.entries(PRICING))
    t[id] = c; return t; })();
...
export function setRuntimeTable(entries, referenceModel, baselineModel) {
    const t = {};
...
export function baselineModelId() { return runtimeBaseline; }
...
export function priceFor(model) {
    ...
    if (norm.includes("qwen") && !norm.includes("deepseek"))
        return runtimeTable["qwen3.8-local"] || null; // metered local Qwen
    // DeepSeek API (corp) unless it's a self-hosted DeepSeek (gguf/4bit/… → local-free below).
    if (norm.includes("deepseek") && !LOCAL_RE.test(norm))
        return runtimeTable["deepseek-v4-flash"] || null; // DeepSeek API (corp)
    if (LOCAL_RE.test(norm))
        return runtimeTable["local-free"] || null; // self-hosted → unpriced (incl. a local DeepSeek GGUF)
    if (norm.includes("opus"))
        return runtimeTable["claude-opus-4.6"] || null;
    if (norm.includes("sonnet"))
        return runtimeTable["claude-sonnet-4.6"] || null;
    if (norm.includes("haiku"))
        return runtimeTable["claude-haiku-4.5"] || null;
    if (norm.includes("grok"))
        return runtimeTable["grok-4.6"] || null;
    return null;
}
...
export function costBreakdown(buckets, card) {
    if (!card)
        return { inputCost: 0, outputCost: 0, cacheReadCost: 0, cacheWriteCost: 0, total: 0 };
    const b = buckets || {};
