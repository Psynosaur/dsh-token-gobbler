    ...
    const walk = (dir) => {
        ...
        for (const e of entries) {
            ...
            if (e.isDirectory()) {
                ...
                if (hasV3 && e.name === "session.jsonl.zstd")
                    continue;
                out.push(full);
            }
        }
...
export function formatVersionName(v) {
    if (v === null)
        return "unknown";
    if (v === 0)
        return "v0 (legacy: top-level chunk records)";
    if (v === 3)
        return "v3 (nested assistant/message stream)";
 ...
 */
export function shapePaths(node, prefix = "", out = []) {
 ...
 */
export function diffShape(a, b) {
    const diffs = [];
    if (a.version !== b.version)
        diffs.push({ kind: "version", detail: `format version ${a.version} -> ${b.version}` });
    for (const t of Object.keys(b.types))
        if (!(t in a.types))
            diffs.push({ kind: "type-added", detail: t });
    for (const t of Object.keys(a.types))
        if (!(t in b.types))
            diffs.push({ kind: "type-removed", detail: t });
    for (const t of Object.keys(a.types)) {
        if (!(t in b.types))
            continue;
        ...
        if (added.length)
            diffs.push({ kind: "field-added", detail: `${t}: +${added.join(", +")}` });
        if (removed.length)
            diffs.push({ kind: "field-removed", detail: `${t}: -${removed.join(", -")}` });
    }
    for (const p of new Set([...Object.keys(a.valueSets), ...Object.keys(b.valueSets)])) {
        ...
        if (added.length)
            diffs.push({ kind: "values-added", detail: `${p}: +${added.join(", ")}` });
        if (removed.length)
            diffs.push({ kind: "values-removed", detail: `${p}: -${removed.join(", ")}` });
    }
    for (const k of Object.keys(b.streamKinds))
        if (!(k in a.streamKinds))
            diffs.push({ kind: "stream-kind-added", detail: k });
    for (const k of Object.keys(a.streamKinds))
        if (!(k in b.streamKinds))
            diffs.push({ kind: "stream-kind-removed", detail: k });
    return diffs;
}
/**
...
export function mergeShapes(shapes) {
    const merged = { version: null, types: {}, counts: {}, valueSets: {}, streamKinds: {}, unparsed: 0 };
    ...
    const betterTypes = new Map();
    const votes = new Map();
    for (const s of shapes) {
        if (s.version !== null)
            votes.set(s.version, (votes.get(s.version) || 0) + 1);
        for (const [t, node] of Object.entries(s.types)) {
            const canon = canonicalShape(node).join("|");
            const prior = betterTypes.get(t);
            if (!prior || canon.length > prior.key.length || (canon.length === prior.key.length && canon > prior.key))
                betterTypes.set(t, { shape: node, key: canon });
        }
        for (const [t, n] of Object.entries(s.counts))
            merged.counts[t] = (merged.counts[t] || 0) + n;
        for (const [p, vals] of Object.entries(s.valueSets)) {
            const set = (merged.valueSets[p] ||= []);
            for (const v of vals)
                if (set.length < VALUE_SET_CAP && !set.some((x) => x === v))
                    set.push(v);
            set.sort((a, b) => String(a).localeCompare(String(b)));
        }
        for (const [k, n] of Object.entries(s.streamKinds))
            merged.streamKinds[k] = (merged.streamKinds[k] || 0) + n;
...
export function parseTrajectoryText(text) {
    ...
    const thinkingTimeByKey = new Map(); // "turn:step" -> { first, last }  // Total thinking (reasoning) text chars per (turn, step), from the fine-grained
    ...
    let openStep = null; // "turn:step" of the step currently streaming (null between steps)
    const msgCharsAtStart = new Map(); // stepKey -> running msg chars at step start
    const sysAtStart = new Map();
    const toolsAtStart = new Map();
    const stepStartSeq = new Map(); // stepKey -> seq of its step/start
    const stepOrder = []; // stepKeys in stream order
    const compactionSeqs = [];
    ...
    const compactionBy = new Map();
    const newCompaction = (seq, time, id) => {
        const ev = { seq, endSeq: null, time, endTime: null, durationMs: null, compactionId: id, hasSummary: false, summaryChars: 0, summaryText: "", shadowedTokens: 0, contextBefore: null, afterTurn: null, afterStep: null, error: null };
    ...
    const assistantMsgChars = (msg) => {
        let n = 0;
        for (const c of (msg?.content || [])) {
            if (!c || typeof c !== "object")
                continue;
            if (c.type === "reasoning" || c.type === "text")
                n += typeof c.text === "string" ? c.text.length : 0;
            else if (c.type === "tool-call")
                n += (typeof c.name === "string" ? c.name.length : 0) + (typeof c.arguments === "string" ? c.arguments.length : 0);
        }
        return n;
    };
    const toolResultChars = (msg) => {
        ...
        for (const c of (msg?.content || []))
            if (c && typeof c === "object" && c.type === "tool-result")
                n += sumTextBlocks(c.content);
        return n;
    };
    const addUserMsg = (id, seq, content) => {
    ...
    const turnStartSeq = new Map();
    const turnOrder = [];
    let activeTurn = null; // the turn currently open (last started, not yet ended)
    const ensureTurn = (turn, seq, time) => {
        ...
        if (!t) {
            ...
            turnOrder.push(turn);
            if (activeTurn == null || turn >= activeTurn)
                activeTurn = turn;
        }
        return t;
    };
    const eventOf = (e) => { out.timeline.events.push(e); };
    /** The turn a seq falls in: the latest turn that had started by then. */
    const turnForSeq = (seq) => {
        let best = null;
        for (const [turn, s] of turnStartSeq)
            if (s <= seq && (best === null || s > (turnStartSeq.get(best) ?? -1)))
                best = turn;
        return best;
    };
    /**
     ...
     */
    const timelineEvent = (pos, seq, time, kind, text) => {
    ...
    const notePrompt = (turnOpt, seq, content) => {
    ...
    const noteResponse = (turnOpt, content) => {
        ...
        if (!text) {
            ...
            for (const c of (Array.isArray(content) ? content : [])) {
                if (c && typeof c === "object" && c.type === "tool-call" && typeof c.name === "string")
                    names.push(String(c.name));
            }
            if (names.length)
                text = oneLine("→ " + [...new Set(names)].join(", "), TRUNC_RESPONSE);
        }
        if (!text)
            return;
        const fromStep = lastEndedStep ? Number(lastEndedStep.split(":")[0]) : null;
        ...
        const turn = turnOpt ?? fromStep ?? activeTurn ?? turnOrder[turnOrder.length - 1] ?? null;
        if (turn == null)
            return;
        const t = ensureTurn(turn, null, null);
        if (!t.response)
            t.response = text; // the turn's opening text (the outline's one-liner)
    };
    const TURN_STATUS = new Set(["completed", "aborted", "blocked", "error", "max-tokens", "interrupted"]);
    const addModelChange = (seq, provider, model) => {
        if (!model)
            return;
        out.modelChanges.push({ seq: typeof seq === "number" ? seq : null, provider: provider ?? null, model });
    };
    const recordUsage = (data, chunk, time, usageOverride) => {
        ...
        if (usageRecordedByKey.has(stepKey))
            return; // dedup: a step that emits both a usage chunk and a
     ...
     */
    const noteStreamStart = (stepKey, time, requestTime) => {
        const had = firstChunkTime.has(stepKey);
        if (!had)
            firstChunkTime.set(stepKey, time);
        if (!ttftByKey.has(stepKey)) {
            const stepStart = stepStartTime.get(stepKey);
            if (typeof stepStart === "number" && time > stepStart)
                ttftByKey.set(stepKey, time - stepStart);
            else if (typeof requestTime === "number" && time > requestTime)
                ttftByKey.set(stepKey, time - requestTime);
        }
        return !had;
    };
    const processStream = (entry, stepKey, requestTime, turn, step) => {
        const chunk = entry.chunk;
        if (chunk) {
            sawChunkByKey.add(stepKey);
            if (typeof entry.time === "number")
                noteStreamStart(stepKey, entry.time, requestTime);
            if (chunk.type === "reasoning-delta") {
                ...
                if (typeof chunk.text === "string" && chunk.text)
                    reasoningDeltaCharsByKey.set(stepKey, (reasoningDeltaCharsByKey.get(stepKey) || 0) + chunk.text.length);
            }
            ...
            if (chunk.type === "usage")
                recordUsage(ref, chunk, entry.time);
            else if (chunk.type === "finish") {
                const resp = chunk.replayState?.response; // v0: the serving model; v3 carries no replayState (see message.source)
                if (resp?.model) {
                    curModel = resp.model;
                    curProvider = resp.provider ?? curProvider;
                }
                if (chunk.usage)
                    recordUsage(ref, chunk, entry.time); // some shapes carry usage on the finish chunk
            }
            return;
        }
        ...
        if (typeof entry.time0 === "number") {
            ...
            if (entry.type === "reasoning-chunks" && texts.length) {
                ...
                let sum = 0;
                if (Array.isArray(entry.dt))
                    for (const d of entry.dt)
                        if (typeof d === "number")
                            sum += d;
                ...
                for (const t of texts)
                    if (typeof t === "string")
                        n += t.length;
                if (n)
                    reasoningCharsByKey.set(stepKey, (reasoningCharsByKey.get(stepKey) || 0) + n);
            }
            if (!sawChunkByKey.has(stepKey))
                noteStreamStart(stepKey, entry.time0, requestTime);
        }
    };
    ...
    const shapeOf = (v, depth) => {
        if (v === null)
            return "null";
        if (Array.isArray(v))
            return "[" + (v.length ? String(shapeOf(v[0], depth + 1)) : "") + "]";
        const t = typeof v;
        if (t !== "object")
            return t;
        if (depth >= 3)
    ...
    const recordValueSets = (v, path) => {
        ...
        if (v !== null && typeof v === "object") {
            for (const [k, child] of Object.entries(v)) {
                if (path === "data" && k === "stream")
                    continue; // the v3 stream is summarized via streamKinds, not per-entry
    ...
    const noteRecord = (r) => {
        ...
        if (!shape.types[type])
            shape.types[type] = shapeOf(r, 0);
        for (const [k, v] of Object.entries(r))
            recordValueSets(v, k);
        if (Array.isArray(r.data?.stream))
            noteStream(r.data.stream); // v3 nested stream
    };
    ...
    for (const ln of lines) {
        ...
        if (cat)
            out.events[cat]++;
        if (r.type === "tool/code-dispatch" && r.data?.name)
            out.tools[r.data.name] = (out.tools[r.data.name] || 0) + 1;
        if (r.type === "tool/call" && r.data?.name) {
            ...
            if (argsChars)
                out.toolCallArgs[r.data.name] = (out.toolCallArgs[r.data.name] || 0) + argsChars;
            if (lastEndedStep) {
        ...
        if (r.type === "session") {
            ...
            if (typeof r.version === "number")
                out.formatVersion = r.version;
        }
        else if (r.type === "request/header") {
            if (typeof r.time === "number")
                lastRequestTime = r.time; // request sent -> TTFT clock starts
            ...
            if (typeof hdr?.system === "string")
                systemChars = hdr.system.length;
            if (Array.isArray(hdr?.tools))
                toolsChars = JSON.stringify(hdr.tools).length;
            const cfg = hdr?.config;
            if (cfg?.model) {
                curModel = cfg.model;
                curProvider = cfg.provider ?? curProvider;
                addModelChange(r.seq, cfg.provider, cfg.model);
            }
            if (systemChars || toolsChars)
                backfillPromptConfig();
        }
        else if (r.type === "request/context") {
            if (typeof r.time === "number")
                lastRequestTime = r.time;
            if (typeof r.data?.contextWindow === "number")
                contextWindow = r.data.contextWindow;
            if (r.data?.model) {
                curModel = r.data.model;
                curProvider = r.data.provider ?? curProvider;
                addModelChange(r.seq, r.data.provider, r.data.model);
            }
        }
            ...
            if (turnNo != null) {
                ...
                if (t.startTime == null && typeof r.time === "number")
                    t.startTime = r.time;
            }
        }
        else if (r.type === "step/start") {
            if (r.data?.turn != null)
                ensureTurn(r.data.turn, typeof r.seq === "number" ? r.seq : null, null).steps++;
            if (typeof r.time === "number" && r.data?.turn != null && r.data?.step != null)
                stepStartTime.set(r.data.turn + ":" + r.data.step, r.time);
            if (r.data?.turn != null && r.data?.step != null) {
                ...
                if (typeof r.seq === "number")
                    stepStartSeq.set(k, r.seq);
                stepOrder.push(k);
            }
        }
        else if (r.type === "step/end") {
            if (typeof r.seq === "number")
                out.stepSeqs.push(r.seq);
            if (r.data?.turn != null && r.data?.step != null)
                lastEndedStep = r.data.turn + ":" + r.data.step;
            ...
            if (isUserStop)
                out.events.userStops++;
            const kind = rs?.kind;
            const detail = isUserStop
                ? "stopped by user"
                : (typeof rs?.error === "string" ? rs.error
                    : (rs?.reason && typeof rs.reason?.kind === "string" ? String(rs.reason.kind) : null));
            if (turnNo != null) {
                const t = ensureTurn(turnNo, typeof r.seq === "number" ? r.seq : null, null);
                t.endTime = typeof r.time === "number" ? r.time : t.endTime;
                t.status = (kind && TURN_STATUS.has(kind) ? kind : "completed");
                t.detail = detail;
            }
            const evKind = isUserStop ? "user-stop" : (kind === "completed" ? "info" : "error");
            ...
            if (title)
                timelineEvent({}, typeof r.seq === "number" ? r.seq : null, typeof r.time === "number" ? r.time : null, "title", "Session title: " + oneLine(title, 120) + (d?.source?.kind ? " (" + d.source.kind + ")" : ""));
        }
            ...
            if (!ev)
                ev = newCompaction(typeof r.seq === "number" ? r.seq : null, typeof r.time === "number" ? r.time : null, cid);
            ev.hasSummary = true;
            ...
            const summaryBlocks = Array.isArray(r.data?.summary)
                ? r.data.summary.filter((b) => b && typeof b === "object" && b.type === "text" && typeof b.text === "string").map((b) => b.text)
                : [];
            ev.summaryText = summaryBlocks.join("\n\n");
            if (typeof r.data?.shadowedTokenCount === "number")
                ev.shadowedTokens = r.data.shadowedTokenCount;
            ...
            timelineEvent(r.data ?? {}, typeof r.seq === "number" ? r.seq : null, typeof r.time === "number" ? r.time : null, "compaction", oneLine("Compaction summary" + (ev.shadowedTokens ? " — " + ev.shadowedTokens + " tokens shadowed" : "") + " (" + ev.summaryChars + " chars)", 200));
        }
        else if (r.type === "compaction/prune") {
            out.prunes++;
            if (typeof r.data?.shadowedTokenCount === "number")
                out.prunedTokens += r.data.shadowedTokenCount;
            timelineEvent(r.data ?? {}, typeof r.seq === "number" ? r.seq : null, typeof r.time === "number" ? r.time : null, "prune", oneLine("Pruned " + (typeof r.data?.shadowedTokenCount === "number" ? r.data.shadowedTokenCount + " tokens" : "context"), 120));
        }
        else if (r.type === "compaction/end") {
            const cid = typeof r.data?.compactionId === "string" ? r.data.compactionId : null;
            let ev = cid ? compactionBy.get(cid) : undefined;
            if (!ev)
                ev = newCompaction(null, null, cid);
            if (ev.seq == null && typeof r.seq === "number")
                ev.seq = r.seq;
            if (typeof r.seq === "number")
                ev.endSeq = r.seq;
            if (ev.time == null && typeof r.time === "number")
                ev.time = r.time;
            if (typeof r.time === "number")
                ev.endTime = r.time;
            if (typeof r.data?.error === "string")
                ev.error = r.data.error;
            if (ev.endTime != null && ev.time != null && ev.endTime > ev.time)
                ev.durationMs = ev.endTime - ev.time;
            ...
            if (ev.error)
                timelineEvent(r.data ?? {}, typeof r.seq === "number" ? r.seq : null, typeof r.time === "number" ? r.time : null, "error", oneLine("Compaction failed — " + ev.error, 160));
            ...
            if (!out.timeline.events.some((e) => e.kind === "system")) {
                const chars = sumTextBlocks(r.data?.message?.content);
                timelineEvent({}, typeof r.seq === "number" ? r.seq : null, typeof r.time === "number" ? r.time : null, "system", "System prompt — " + chars + " chars" + (typeof r.data?.turn === "number" ? " · first sent in turn " + r.data.turn : ""));
            }
            ...
            for (const m of (r.data?.inserted || [])) {
                addUserMsg(m?.id, typeof r.seq === "number" ? r.seq : 0, m?.content);
                notePrompt(m?.turn ?? null, typeof r.seq === "number" ? r.seq : 0, m?.content);
            }
        }
        else if (r.type === "assistant/message") {
            msgCharsRunning += assistantMsgChars(r.data?.message);
            noteResponse(r.data?.turn ?? null, r.data?.message?.content);
            ...
            if (typeof r.data?.turn === "number" || typeof r.data?.step === "number") {
                ...
                if (Array.isArray(r.data?.stream))
                    for (const e of r.data.stream)
                        processStream(e, k, reqTime, r.data.turn, r.data.step);
                ...
                if (src?.kind === "model" && src.model) {
                    curModel = src.model;
                    curProvider = src.provider ?? curProvider;
                }
                ...
                if (r.data?.usage && !usageRecordedByKey.has(k)) {
                    if (!firstChunkTime.has(k) && typeof r.time === "number")
                        firstChunkTime.set(k, r.time);
                    recordUsage(r.data, { usage: r.data.usage }, r.time, r.data.usage);
                }
            }
            ...
            if (r.data?.turn != null && r.data?.step != null) {
                processStream({ type: r.type, time0: r.time0, texts: r.data.texts }, stepKeyOf(r.data.turn, r.data.step), lastRequestTime);
            }
        }
        else if (r.type === "assistant/chunk") {
            if (!r.data?.chunk)
                continue;
            processStream({ ...r.data, chunk: r.data.chunk, time: r.time }, stepKeyOf(r.data.turn, r.data.step), lastRequestTime);
            lastRequestTime = null; // consumed by this step's stream
        }
    }
    ...
    stepOrder.forEach((k, i) => {
        ...
        let compSeq = 0;
        for (const c of compactionSeqs)
            if (c < startSeq)
                compSeq = Math.max(compSeq, c);
        let userChars = 0;
        for (const um of uniqueUserMsgs) {
            if (!um.entry || um.seq <= compSeq)
                continue;
    ...
    for (const ev of compactionBy.values()) {
        ...
        let anchor = null;
        for (const k of stepOrder)
            if ((stepStartSeq.get(k) || 0) < endSeq)
                anchor = k;
        if (anchor) {
            const parts = anchor.split(":");
            ev.afterTurn = Number(parts[0]);
            ev.afterStep = Number(parts[1]);
        }
    }
    ...
    const SESSION_LEVEL_EVENTS = new Set(["system", "title", "info", "session"]);
    for (const e of out.timeline.events) {
        if (e.turn != null || SESSION_LEVEL_EVENTS.has(e.kind))
            continue;
...
export function zstdMultiFrameDecompress(buf) {
    ...
    for (const s of starts) {
        try {
            out.push(zstdDecompressSync(buf.subarray(s)));
        }
...
export function loadDiskCache(dshHome) {
    ...
    let data;
    try {
        data = JSON.parse(readFileSync(_diskCachePath, "utf8"));
    }
    catch {
        return { loaded: 0 };
    }
    ...
    for (const [filePath, entry] of Object.entries(data.entries || {})) {
        if (isDropped(filePath))
            continue; // an import that was removed/resynced since the cache was written
...
export function saveDiskCache(dshHome) {
    ...
    for (const [filePath, entry] of parseCache) {
        if (isDropped(filePath)) {
            ...
            continue;
        } // an import that was removed/resynced
 ...
 */
const droppedPrefixes = new Set();
const isDropped = (filePath) => {
 ...
 */
export function invalidateTrajectoryCache(prefix) {
    if (!prefix)
        return 0;
    ...
    for (const key of [...parseCache.keys()]) {
        if (key === prefix || key.startsWith(root)) {
...
export function clearDiskCache(dshHome) {
    ...
    if (dshHome) {
        try {
            const p = trajectoryCachePath(dshHome);
            if (existsSync(p))
                renameSync(p, p + ".old");
        }
        catch { /* ok */ }
