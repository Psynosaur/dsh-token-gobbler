...
export const emptyEvents = () => ({ steps: 0, toolCalls: 0, toolSubCalls: 0, userMessages: 0, assistantMessages: 0, turns: 0, compactions: 0, retries: 0, approvals: 0, todos: 0, commands: 0, userStops: 0 });
export function parseTrajectoryText(text) {
    const lines = String(text).split("\n").filter(Boolean);
    const out = { meta: null, usage: [], modelCounts: {}, modelChanges: [], stepSeqs: [], events: emptyEvents(), tools: {}, toolCalls: {}, toolCallArgs: {}, stepTools: {}, stepToolArgs: {}, decode: { tokens: 0, ms: 0, steps: 0 }, prefill: { tokens: 0, ms: 0, steps: 0 }, systemChars: 0, toolsChars: 0, contextWindow: null, stepContext: {}, compactions: [], prunes: 0, prunedTokens: 0, postCompaction: {} };
    let curModel = null;
    ...
    const noteThinking = (stepKey, t) => {
        ...
        if (!e)
            thinkingTimeByKey.set(stepKey, { first: t, last: t });
        else {
            if (t < e.first)
                e.first = t;
            if (t > e.last)
    ...
    for (const ln of lines) {
        ...
        if (r.type === "session") {
            ...
            if (rs && rs.kind === "aborted" && rs.reason && rs.reason.kind === "user")
            ...
            if (r.data?.turn != null && r.data?.step != null) {
                ...
                for (const t of (r.data.texts || []))
                    if (typeof t === "string")
                        reasoningCharsByKey.set(k, (reasoningCharsByKey.get(k) || 0) + t.length);
            }
        }
            ...
            if (typeof r.time === "number" && !firstChunkTime.has(stepKey)) {
                ...
                if (typeof stepStart === "number" && r.time > stepStart)
                    ttftByKey.set(stepKey, r.time - stepStart);
                else if (typeof lastRequestTime === "number" && r.time > lastRequestTime)
                    ttftByKey.set(stepKey, r.time - lastRequestTime);
            ...
            if (chunk.type === "usage")
                recordUsage(r.data, chunk, r.time);
                ...
                if (chunk.usage)
                    recordUsage(r.data, chunk, r.time); // some shapes carry usage on the finish chunk
            }
        }
    }
