...
(() => {
  ...
  var PREV_KEYS = ["tg:cache:v1", "tg:cache:v2"];
      ...
      loadAmCharts().then((w) => {
        ...
        if ((props.kind || "column") === "scatter") {
          ...
          for (const s of [...props.series.filter((x) => x.line), ...props.series.filter((x) => !x.line)]) {
            ...
            dataMeta.push({ ser, k: s.regime != null ? s.regime : s.label, mk: s.bullet ? s.key : void 0, ctxs: rows.map((r) => Number(r[props.xField || "x"]) || 0) });
          }
          let tipSer = null;
  ...
  var combinedDrawer = (s, opts = {}) => {
    ...
    regimeKeys.forEach((r, i) => {
      ...
      for (const m2 of PERF_METRICS) {
        perfSeries.push({ key: m2.key, label: m2.name, color: m2.color, unit: m2.unit, axis: 1, regime: r, bullet: "circle", data: rows, radius: m2.radius });
      }
    });
    const perfTipRows = regimeKeys.flatMap((r) => regimeRows[r]);
    const perfRules = (s.compactionEvents || []).filter((c) => c.contextBefore != null && c.afterTurn != null && c.afterStep != null && stepG[c.afterTurn + ":" + c.afterStep] != null);
