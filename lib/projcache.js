...
const cut = (s) => (typeof s === "string" && s.length > TRUNC ? s.slice(0, TRUNC) + "…" : s);

/** Extract one session record ({identity, rows}) into a report row + rich metadata. */
function extractSession(id, entry) {
 ...
 */
export function readProjcache(storePath) {
  let entries = null; // { [id]: {identity, rows} }
  try {
  ...
  for (const [id, entry] of Object.entries(entries)) {
    const s = extractSession(id, entry?.record || entry);
    if (s.allTokens > 0) nonZero++;
    if (s.buckets) for (const k of Object.keys(totals)) totals[k] += s.buckets[k];
    sessions.push(s);
  }
