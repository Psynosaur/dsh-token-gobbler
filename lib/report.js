...
export function resolvePaths(opts = {}) {
    ...
    if (storePath) {
        ...
        try {
            if (statSync(sibling).isFile() || statSync(sibling).isDirectory())
                otherStorePath = sibling;
        }
        catch { /* no sibling */ }
    }
    else {
...
const emptyLoadStats = (id) => ({ id, sessions: 0, tokens: 0, files: 0, withUsage: 0, duplicates: 0, synthetic: 0 });
/** The local home first, then every enabled import (unreadable ones included, flagged). */
export function resolveAllSources(opts = {}) {
...
function archivedIdsOf(all) {
    const out = new Set();
    for (const s of all)
        if (s.home)
            for (const id of readArchivedSessions(s.home))
                out.add(id);
    return out;
}
/** Registry entries + the load counters of the report being served. */
function buildSourceViews(dshHome, all, stats) {
 ...
 */
export function readArchivedSessions(dshHome) {
    try {
 ...
 */
export function reprocessTrajectories(opts = {}) {
 ...
 */
function defaultReference(models) {
    if ((models || []).some((m) => m.id === "claude-opus-4.6"))
...
function defaultBaseline(models) {
 ...
 */
export function loadPricing(dshHome, discovered) {
    ...
    if (hasModels) {
        ...
        const byId = new Map();
        for (const e of pricing.builtinEntries())
            byId.set(e.id, e);
        for (const e of pricing.seedEntries(discovered))
            if (!byId.has(e.id))
                byId.set(e.id, e);
        models = [...byId.values()];
        source = "seeded";
...
export function discoverModels(dshHome, opts = {}) {
    const map = new Map();
    const roots = resolveAllSources({ ...opts, dshHome }).map((s) => s.sessionsRoot).filter((r) => !!r && existsSync(r));
    for (const sessionsRoot of roots)
        for (const f of trajectory.findTrajectoryFiles(sessionsRoot)) {
            let t;
            try {
                t = trajectory.readTrajectory(f);
            }
            catch {
                continue;
            }
...
function discoverFromTimelines(timelines) {
    const map = new Map();
 ...
 */
export function discoverModelCards(dshHome, opts = {}) {
 ...
 */
export function discoverLocalModels(dshHome, opts = {}) {
...
export function savePricing(dshHome, body = {}) {
    ...
    const models = [];
    const seen = new Set();
    for (const e of (isArray(body.models) ? body.models : [])) {
    ...
    if (!models.length)
        throw new Error("models must be a non-empty array of {id, input, output, cacheRead, cacheWrite}");
    ...
    if (!referenceModel || !seen.has(referenceModel))
        throw new Error("referenceModel must be one of the model ids in the table");
    const refEntry = models.find((m) => m.id === referenceModel);
    if (refEntry && refEntry.local)
        throw new Error("referenceModel must be a corp (non-local) model — local compute is valued against a billed rate card");
    ...
    if (body.baselineModel != null && String(body.baselineModel).trim()) {
        ...
        if (!seen.has(b))
            throw new Error("baselineModel must be one of the model ids in the table");
        const bEntry = models.find((m) => m.id === b);
        if (!bEntry || !bEntry.local)
            throw new Error("baselineModel must be a local model — the comparison baseline is your home-lab card");
        baselineModel = b;
    }
    else {
...
export function priceTotals(totals, candidates = DEFAULT_CANDIDATES) {
    const out = [];
    for (const c of candidates) {
        const card = pricing.priceFor(c.id);
        if (!card) {
            out.push({ id: c.id, label: c.label, priced: false, cost: null });
            continue;
        }
...
export function attributeSession(totals, modelChanges, stepSeqs, fallbackModel) {
    ...
    const mkBuckets = (share) => { const b = emptyLikeBuckets(); for (const k of BUCKETS)
        b[k] = (t[k] || 0) * share; return b; };
    if (!changes.length) {
        const m = fallbackModel || "(unattributed)";
        return [{ model: m, provider: null, steps: 0, share: 1, buckets: { ...t }, allTokens: all }];
    }
    const countBy = new Map();
    const activeAt = (seq) => { let best = changes[0]; for (const c of changes) {
        if ((c.seq ?? 0) <= seq)
            best = c;
        else
            break;
    ...
    if (steps.length) {
        for (const s of steps) {
            const c = activeAt(s);
            const e = countBy.get(c.model) || { steps: 0, provider: c.provider };
            e.steps++;
            e.provider = c.provider || e.provider;
            countBy.set(c.model, e);
        }
    }
    else {
        const c = changes[0];
    ...
    for (const s of pc.sessions) {
        ...
        if (usage && usage.length) {
            ...
            for (const u of usage) {
                ...
                let e = byM.get(key);
                if (!e) {
                    e = { key, provider: u.provider || null, steps: 0, buckets: pricing.emptyBuckets(), decodeTokens: 0, decodeMs: 0, decodeSteps: 0, prefillTokens: 0, prefillMs: 0, prefillSteps: 0, ctxTokens: 0, reasoningTokens: 0 };
                    byM.set(key, e);
                }
                e.provider = e.provider || u.provider;
                for (const k of BUCKETS)
                    e.buckets[k] += u.buckets[k];
                if (u.decodeMs != null) {
                    e.decodeTokens += u.buckets.outputTokens || 0;
                    e.decodeMs += u.decodeMs;
                    e.decodeSteps++;
                }
                if (u.ttftMs != null) {
                    e.prefillTokens += u.buckets.uncachedInputTokens || 0;
                    e.prefillMs += u.ttftMs;
                    e.prefillSteps++;
                }
            ...
            for (const m of raw) {
                ...
                let e = byKey.get(key);
                if (!e) {
                    e = { key, provider: m.provider, steps: 0, share: 0, buckets: pricing.emptyBuckets() };
                    byKey.set(key, e);
                }
...
export function aggregateContrib(contrib, modelAliasMap) {
    const map = new Map();
    for (const c of contrib || []) {
        for (const m of c.models || []) {
            let e = map.get(m.key);
            if (!e) {
                e = { key: m.key, provider: m.provider, sessions: new Set(), steps: 0, exact: true, decodeTokens: 0, decodeMs: 0, decodeSteps: 0, prefillTokens: 0, prefillMs: 0, prefillSteps: 0, ctxTokens: 0, reasoningTokens: 0, ...pricing.emptyBuckets() };
                map.set(m.key, e);
            }
            e.provider = e.provider || m.provider;
            e.sessions.add(c.id);
...
function sessionFromTrajectory(id, source, t) {
...
function loadSources(opts = {}) {
    ...
    const sourceStats = new Map(all.map((s) => [s.id, emptyLoadStats(s.id)]));
    ...
    const toolCallArgsById = new Map(); // sessionId -> { toolName: total argsChars }
    const stepToolArgsById = new Map(); // sessionId -> { "turn:step": { toolName: argsChars } }
    const stepToolsById = new Map(); // sessionId -> { "turn:step": [toolNames] }
    const stepCtxById = new Map(); // sessionId -> per-step context allocation (chars) + window limit + post-compaction regime index per step
    const timelineById = new Map(); // sessionId -> per-turn prompt/response/outcome + the session event timeline
    const compactionsById = new Map(); // sessionId -> compaction events (existence + impact) + prune totals
    const p2pById = new Map(); // sessionId -> P2P enablement evidence + mention volume
    ...
    const claimed = new Map(); // sessionId -> owning source id
    const pcSessions = [];
    for (const src of all) {
        const st = sourceStats.get(src.id);
        if (!src.storePath)
            continue;
        let raw;
        try {
            raw = projcache.readProjcacheMerged({ primary: src.storePath, secondary: src.otherStorePath }, src.id);
        }
        catch {
            continue;
    ...
    // ── PASS 2 · trajectories ─────────────────────────────────────────────
    const seenTrajectory = new Set(); // one copy per session id (the first home wins)
    let cacheLoaded = false;
    let stats0 = null;
    for (const src of all) {
        const st = sourceStats.get(src.id);
        if (!src.sessionsRoot || !existsSync(src.sessionsRoot))
            continue;
        if (!cacheLoaded) {
            cacheLoaded = true;
            const _cr = trajectory.loadDiskCache(dshHome);
            if (_cr.loaded)
                process.stderr.write("[token-gobbler] trajectory cache: " + _cr.loaded + " entries from disk" + "\n");
        ...
        for (const f of files) {
            ...
            if (!id)
                continue;
            if (seenTrajectory.has(id))
                continue; // a second copy of the same session (another home, or the legacy sibling)
            ...
            if (t.modelChanges.length)
                traj.withModelTimeline++;
            if (t.usage.length) {
                traj.withUsage++;
                st.withUsage++;
                traj.usageRecords += t.usage.length;
                usageById.set(id, t.usage);
            }
            if (t.events) {
                eventsById.set(id, t.events);
                for (const k of Object.keys(aggEvents))
                    aggEvents[k] += t.events[k] || 0;
            }
            if (t.tools && Object.keys(t.tools).length) {
                toolsById.set(id, t.tools);
                for (const [n, c] of Object.entries(t.tools))
                    aggTools[n] = (aggTools[n] || 0) + c;
            }
            if (t.toolCalls && Object.keys(t.toolCalls).length) {
                toolCallsById.set(id, t.toolCalls);
                for (const [n, c] of Object.entries(t.toolCalls))
                    aggToolCalls[n] = (aggToolCalls[n] || 0) + c;
            }
            if (t.stepTools && Object.keys(t.stepTools).length)
                stepToolsById.set(id, t.stepTools);
            if (t.toolCallArgs && Object.keys(t.toolCallArgs).length)
                toolCallArgsById.set(id, t.toolCallArgs);
            if (t.stepToolArgs && Object.keys(t.stepToolArgs).length)
                stepToolArgsById.set(id, t.stepToolArgs);
            if (t.stepContext && Object.keys(t.stepContext).length)
                stepCtxById.set(id, { byStep: t.stepContext, window: t.contextWindow ?? null, post: t.postCompaction || {} });
            if (t.timeline && (t.timeline.turns.length || t.timeline.events.length))
                timelineById.set(id, t.timeline);
            if ((t.compactions && t.compactions.length) || t.prunes)
                compactionsById.set(id, { events: t.compactions || [], prunes: t.prunes || 0, prunedTokens: t.prunedTokens || 0 });
            if (t.p2p || t.p2pMentions)
                p2pById.set(id, { p2p: !!t.p2p, mentions: t.p2pMentions || 0 });
        }
    }
    if (cacheLoaded && stats0) {
        ...
        if (traj.cache.recomputed > 0)
            trajectory.saveDiskCache(dshHome);
    }
    // ── merge ─────────────────────────────────────────────────────────────
    ...
    for (const s of pcSessions) {
        if (s.buckets)
            for (const k of BUCKETS)
                pcTotals[k] += s.buckets[k];
        if (s.allTokens > 0)
            nonZero++;
    }
    pcSessions.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
    const pc = { sessions: pcSessions, totals: pcTotals, count: pcSessions.length, nonZero };
...
function candidatesFromTable(models, baselineModel) {
    const paid = [];
    let localBaseline = null;
    const wants = (baselineModel || "").trim().toLowerCase();
    for (const m of models || []) {
        if (!m || !m.id || m.id === "local-free")
            continue; // local-free is an unpriced fallback marker, not a candidate
        if (m.local) {
            // The user-set baseline wins (when it's still a local row); else the first local row.
            if (!localBaseline)
                localBaseline = { id: m.id, label: m.label + " — what you ran" };
            else if (m.id === wants)
                localBaseline = { id: m.id, label: m.label + " — what you ran" };
        }
        else
            paid.push({ id: m.id, label: m.label || m.id });
    }
...
export function buildReport(opts = {}) {
    ...
    for (const m of byModel) {
        const card = pricing.priceFor(m.model);
        if (card) {
            actualCost += pricing.costFor(m, card);
            pricedModels++;
        }
    }
    ...
    const sumRows = (rows) => { const b = pricing.emptyBuckets(); for (const m of rows)
        for (const k of BUCKETS)
            b[k] += m[k] || 0; return b; };
    ...
    const corpSessionIds = new Set();
    for (const c of contrib) {
        let local = false, corp = false;
        for (const m of c.models || []) {
            if (m.allTokens <= 0)
                continue;
            if (pricing.kindFor(m.provider, m.key) === "corp")
                corp = true;
            else
                local = true;
        }
        if (local)
            wfhSessionIds.add(c.id);
        if (corp)
            corpSessionIds.add(c.id);
    }
...
export function buildBreakdown(opts = {}) {
    ...
    const tps = (tok, ms) => (ms && ms > 0 ? Math.round((tok / (ms / 1000)) * 10) / 10 : null);
    const bySession = pc.sessions
        ...
        const ml = (key) => pricing.priceFor(key)?.label || key;
        ...
        if (models.length) {
            ...
            for (const m of models) {
                const card = pricing.priceFor(m.key);
                if (card) {
                    sum += pricing.costFor(m.buckets, card);
                    hasCard = true;
                }
            }
        ...
        if (_usage && _usage.length && _stepTools && Object.keys(_stepTools).length) {
            const byTool = new Map(); // toolName -> { payload: [], reasoning: [] }
            for (const u of _usage) {
                ...
                const layer = stepToolArgsById.get(s.id) ?? {};
                const stepArgs = layer[stepKey] ?? {};
        ...
        // Distribute the session total tokens uniformly across tool-calling steps.
            ...
            const callCount = new Map(); // toolName -> stepCount
            for (const tools of Object.values(_stepTools)) {
                for (const tool of tools)
                    callCount.set(tool, (callCount.get(tool) || 0) + 1);
            }
        ...
        if (steps && steps.length) {
            ...
            for (const st of steps) {
                ...
                const stepEntry = { ...st, tools, parallel: tools.length > 1 };
    ...
    for (const row of bySession) {
        ...
        if (!d) {
            d = { date: row.date, sessions: 0, cost: null, ...pricing.emptyBuckets() };
            dayMap.set(row.date, d);
        }
        d.sessions++;
        if (row.cost != null)
            d.cost = (d.cost || 0) + row.cost;
        for (const k of BUCKETS)
            d[k] += row[k];
    }
    ...
    const toolTokensAggregate = (function () {
        const byTool = new Map(); // toolName -> { contextTokens: [], reasoning: [], inputTokens: [], sessions: Set }
        for (const row of bySession) {
            if (!row.toolTokens)
                continue;
            for (const t of row.toolTokens) {
                ...
                if (!e) {
                    e = { tokens: [], reasoning: [], inputTokens: [], sessions: new Set() };
                    byTool.set(t.tool, e);
                }
 ...
 */
export function getCompactionDetail(sessionId, index) {
...
export function buildPerformance(opts = {}) {
    ...
    const sessionRows = pc.sessions
        ...
        const c = contribById.get(s.id);
        if (!c || !c.exact)
            return null; // only exact-usage sessions have timing
...
function defaultSourceLabel(layout) {
    ...
    if (last === ".dsh" || last.startsWith(".dsh")) {
        ...
        if (parts.length > 1 && last === ".dsh")
            return parts[parts.length - 2] + " · .dsh";
        return last;
    }
    return last;
}
const asOs = (v) => v === "linux" || v === "windows" || v === "macos" ? v : (v === "auto" || v == null || v === "" ? null : null);
const viewOf = (s, over = {}) => ({
    id: s.id,
    label: s.label,
    os: s.os,
...
function scanRegistered(s) {
    ...
    if (s.osAuto && !error) {
        ...
        if (detected.os !== "unknown")
            os = detected.os; // a home that learned what it is
    }
 ...
 */
export function listImportSources(dshHome) {
...
export function addImportSource(dshHome, input) {
    ...
    const error = imports.layoutError(layout);
    if (error)
        throw new Error(error + ": " + layout.root);
    const reg = imports.readSourceRegistry(dshHome);
    const already = reg.sources.find((s) => imports.resolveSourceLayout(s.path).root === layout.root);
    if (already)
        throw new Error('that path is already imported as "' + already.label + '"');
...
export function removeImportSource(dshHome, id) {
    const reg = imports.readSourceRegistry(dshHome);
    const src = reg.sources.find((s) => s.id === id);
    if (!src)
        return { removed: false, dropped: 0 };
...
export function updateImportSource(dshHome, id, patch) {
    const reg = imports.readSourceRegistry(dshHome);
    const src = reg.sources.find((s) => s.id === id);
    if (!src)
 ...
 */
export function resyncImportSource(dshHome, id) {
    const reg = imports.readSourceRegistry(dshHome);
    const src = reg.sources.find((s) => s.id === id);
    if (!src)
        throw new Error("unknown source: " + id);
...
export function scanImportCandidates(dshHome, roots) {
    const reg = imports.readSourceRegistry(dshHome);
