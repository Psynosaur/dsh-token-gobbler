...
export function loadPricing(dshHome, discovered) {
  ...
  let models, source;
  if (hasModels) { models = file.models; source = "file"; }
  ...
  return { path: p, fromFile: !!hasModels, seeded: source === "seeded", referenceModel, models };
}
...
export function discoverModels(dshHome, opts = {}) {
  ...
  for (const f of trajectory.findTrajectoryFiles(sessionsRoot)) {
    let t; try { t = trajectory.readTrajectory(f); } catch { continue; }
    if (!t) continue;
    for (const mc of t.modelChanges || []) {
      if (!mc.model) continue;
      const id = pricing.normalizeModel(mc.model);
      if (!id) continue;
      const e = map.get(id) || { id, label: mc.model, provider: null, steps: 0 };
...
function discoverFromTimelines(timelines) {
  ...
  for (const [id, tl] of timelines) {
    for (const mc of tl.modelChanges || []) {
      ...
      if (!idm) continue;
      const e = map.get(idm) || { id: idm, label: mc.model, provider: null, steps: 0 };
      e.provider = e.provider || mc.provider;
      e.steps += 1;
      map.set(idm, e);
    }
 ...
 * qwen baseline). Used by the Pricing tab's "scan trajectories for local models"
 * button. Rates are resolved family-wise (Qwen -> metered, other self-hosted ->
 * local-free) so the added rows are immediately costed.
 */
export function discoverLocalModels(dshHome, opts = {}) {
  const found = discoverModels(dshHome, opts);
  // Ensure the runtime rate table is populated so family rates resolve (Qwen ->
  // the metered qwen card, other self-hosted -> local-free). Cheap: one pricing
  // file read, or builtin+discovered seeding when nothing is saved yet.
...
function loadSources(opts = {}) {
  ...
  return { dshHome, sessionsRoot, storePath, storeKind, priceCfg, pc, traj, defaultModel, contrib, byModel, eventsById, toolsById, toolCallsById, toolCallArgsById, stepToolArgsById, stepToolsById, usageById, aggEvents, aggTools, aggToolCalls };
}
...
function candidatesFromTable(models) {
  ...
  for (const m of models || []) {
    ...
    if (m.local) { if (!localBaseline) localBaseline = { id: m.id, label: m.label + " — what you ran" }; }
...
export function buildBreakdown(opts = {}) {
  const { pc, traj, defaultModel, contrib, byModel, eventsById, toolsById, toolCallsById, toolCallArgsById, stepToolArgsById, stepToolsById, usageById, aggEvents, aggTools, aggToolCalls } = loadSources(opts);
    ...
    .map((s) => {
      ...
      const dMs = models.reduce((n, m) => n + (m.decodeMs || 0), 0);
        ...
        if (_usage && _usage.length && _stepTools && Object.keys(_stepTools).length) {
          ...
          for (const u of _usage) {
            ...
            const stepArgs = (stepToolArgsById.get(s.id) || {})[stepKey] || {};
      ...
      if (steps && steps.length) {
        ...
        for (const st of steps) {
          ...
          if (!byTurn.has(st.turn)) byTurn.set(st.turn, []);
          byTurn.get(st.turn).push(stepEntry);
        }
